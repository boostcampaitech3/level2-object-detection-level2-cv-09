...
def main(args):
    with open(f'{args.path}train.json', 'rt', encoding='UTF-8') as annotations:
        ...
        for i, (train_idx, val_idx) in enumerate(cv.split(groups, y, groups)):
            ...
            save_coco(f'{args.path}cv{args.split}_train{i}.json', info, licenses, list(images[train_idx]), filter_annotations(annotations, list(images[train_idx])), categories)
            save_coco(f'{args.path}cv{args.split}_val{i}.json', info, licenses, list(images[val_idx]), filter_annotations(annotations, list(images[val_idx])), categories)
